...
pub fn extract_tag_strips_v_test() {
  let body = "{\"tag_name\": \"v5.0.12\", \"name\": \"5.0.12\"}"
  github.extract_tag(body) |> should.equal(Ok("5.0.12"))
}

pub fn extract_tag_without_v_test() {
  let body = "{\"tag_name\": \"5.0.12\"}"
  github.extract_tag(body) |> should.equal(Ok("5.0.12"))
}
...
pub fn extract_tag_picks_first_match_test() {
  ...
  let body = "{\"tag_name\": \"v5.0.12\", \"author\": {\"tag_name\": \"junk\"}}"
  github.extract_tag(body) |> should.equal(Ok("5.0.12"))
}

pub fn extract_tag_missing_returns_error_test() {
  github.extract_tag("{\"name\": \"5.0.12\"}") |> should.equal(Error(Nil))
