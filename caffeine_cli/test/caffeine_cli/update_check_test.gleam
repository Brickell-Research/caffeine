...
pub fn extract_tag_strips_v_test() {
  let body = "{\"tag_name\": \"v5.4.2\", \"name\": \"5.4.2\"}"
  github.extract_tag(body) |> should.equal(Ok("5.4.2"))
}

pub fn extract_tag_without_v_test() {
  let body = "{\"tag_name\": \"5.4.2\"}"
  github.extract_tag(body) |> should.equal(Ok("5.4.2"))
}
...
pub fn extract_tag_picks_first_match_test() {
  ...
  let body = "{\"tag_name\": \"v5.4.2\", \"author\": {\"tag_name\": \"junk\"}}"
  github.extract_tag(body) |> should.equal(Ok("5.4.2"))
}

pub fn extract_tag_missing_returns_error_test() {
  github.extract_tag("{\"name\": \"5.4.2\"}") |> should.equal(Error(Nil))
