...
pub fn all() -> Dict(String, Explanation) {
  [
    ...
    Explanation(
      ...
      title: "parse error",
      summary:
        "The tokenizer or parser couldn't make sense of a .caffeine file. Something about the syntax didn't match what Caffeine expected at that position.",
      causes: [
      ...
      fix:
        "Re-read the line shown in the error. Parse errors point at the exact column where the parser gave up — the fix is usually within one or two tokens of that caret.",
    ...
    Explanation(
      ...
      summary:
        "The file parsed, but its shape is inconsistent: an extendable is referenced that doesn't exist, a type alias is defined twice, a circular alias chain, a refinement value isn't the right kind of literal, and so on.",
      causes: [
      ...
      fix:
        "The error message names the offending identifier. Search the file for that name to find the definition and references; either add the missing definition, remove the duplicate, break the cycle, or correct the type.",
    ...
    Explanation(
      ...
      summary:
        "The linker tried to re-parse a fragment (typically a templated value or a standard-library artifact reference) and failed. This is distinct from E100 — the original file parsed fine, but a derived fragment didn't.",
      causes: [
        "An artifact reference whose template body is malformed.",
        "A standard-library artifact the linker doesn't recognize.",
      ],
      fix:
        "Check the artifact name and argument list against `caffeine artifacts`. If you wrote a custom artifact reference, verify its template syntax.",
    ...
    Explanation(
      ...
      summary:
        "A literal value (a number, string, list, or record) didn't match the declared type of the field it was assigned to.",
      causes: [
      ...
      fix:
        "Look at the type declaration for the field named in the error and adjust the value to match. `caffeine types` shows the full type-system reference.",
    ...
    Explanation(
      ...
      summary:
        "Two measurements or expectations in the workspace have the same name. Caffeine requires names to be unique across all files, not just within one file.",
      causes: [
      ...
      fix:
        "Rename one of the duplicates. If the two definitions should be the same thing, consolidate them into a single file and remove the other.",
    ...
    Explanation(
      ...
      summary:
        "An expectation references a measurement whose vendor (Datadog, Honeycomb, etc.) Caffeine couldn't resolve, or the vendor is supported but configured in a way the linker rejected.",
      causes: [
      ...
      fix:
        "Confirm the measurement file is under a vendor-named directory (`measurements/datadog/...`) and that the vendor's required fields are present.",
    ...
    Explanation(
      ...
      summary:
        "A templated value (like `$window_default` or an artifact argument) couldn't be parsed. Semantic analysis tried to resolve it and the template body itself was syntactically wrong.",
      causes: [
      ...
      fix:
        "Locate the template named in the error and verify its body is well-formed. Artifact templates live in the standard library — `caffeine artifacts` lists them.",
    ...
    Explanation(
      ...
      summary:
        "A template parsed, but couldn't be resolved against the current context. Usually this means a reference to a value the template expected to exist, but didn't.",
      causes: [
      ...
      fix:
        "Read the error message for the missing name and ensure it's defined in the surrounding block (the measurement or expectation), or add the missing argument to the artifact call.",
    ...
    Explanation(
      ...
      summary:
        "An expectation's `relations` field names a dependency (another expectation or measurement) that doesn't exist in the workspace.",
      causes: [
      ...
      fix:
        "Check that the dependency is defined and lives under the expectations or measurements directory Caffeine is compiling. Either fix the name, add the missing definition, or remove the stale reference.",
    ...
    Explanation(
      ...
      summary:
        "The code generator hit a vendor it doesn't have a Terraform template for. This is the catch-all code — the specific vendor codes (E502–E505) are more informative when they apply.",
      causes: [
      ...
      fix:
        "Check `caffeine artifacts` or the project's supported-vendor list. If the vendor should be supported, this is a compiler bug — please file it.",
    ...
    Explanation(
      ...
      summary:
        "Code generation couldn't assemble the metric query for an SLO. The SLO's `query` field references something (a measurement, a template parameter) that didn't resolve to a concrete value at codegen time.",
      causes: [
      ...
      fix:
        "Inspect the SLO's `query` field. Ensure every template parameter has a value (either a default or one passed via the expectation) and every measurement name resolves.",
    ...
    Explanation(
      ...
      summary:
        "Caffeine couldn't generate the Terraform resource for a Datadog-backed SLO or monitor. The measurement or expectation is valid on its own, but something specific to the Datadog provider template rejected it.",
      causes: [
      ...
      fix:
        "Check the Datadog provider docs for the specific resource shape, and ensure the expectation declares every field the Datadog template requires.",
    ...
    Explanation(
      ...
      summary:
        "Caffeine couldn't generate the Terraform resource for a Honeycomb-backed measurement. The measurement or expectation is valid on its own, but the Honeycomb provider template rejected a specific field.",
      causes: [
        "Required Honeycomb-specific fields missing (dataset, query spec).",
        "Derived-column reference that doesn't exist in the dataset.",
      ],
      fix:
        "Check the Honeycomb provider docs for the resource shape, and ensure the dataset and any derived columns referenced actually exist in Honeycomb.",
    ...
    Explanation(
      ...
      summary:
        "Caffeine couldn't generate the Terraform resource for a Dynatrace-backed measurement. The measurement or expectation is valid on its own, but the Dynatrace provider template rejected a specific field.",
      causes: [
      ...
      fix:
        "Check the Dynatrace provider docs for the resource shape and verify the metric selector is well-formed.",
    ...
    Explanation(
      ...
      summary:
        "Caffeine couldn't generate the Terraform resource for a New Relic–backed measurement. The measurement or expectation is valid on its own, but the New Relic provider template rejected a specific field.",
      causes: [
      ...
      fix:
        "Check the New Relic provider docs for the resource shape and verify the NRQL query is valid by running it in One.",
    ...
    Explanation(
      ...
      summary:
        "A Caffeine Query Language expression parsed but couldn't be resolved against the workspace. Typically this is a reference to a field or measurement that doesn't exist in scope.",
      causes: [
      ...
      fix:
        "Check the names in the CQL expression against the measurement or expectation fields they're pulling from. The error message names the unresolved symbol.",
    ...
    Explanation(
      ...
      summary:
        "A Caffeine Query Language expression didn't parse. The CQL tokenizer or parser gave up on the syntax before it could build a tree.",
      causes: [
      ...
      fix:
        "Re-read the CQL expression at the column the error points to. Most CQL syntax errors are within one or two tokens of that position.",
